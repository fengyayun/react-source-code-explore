import { REACT_TEXT, REACT_FORWARD_REF, REACT_CONTEXT, REACT_PROVIDER, REACT_MEMO, REACT_FRAGMENT } from './ReactSymbol'
...
export function useState(initialState){
  hookStates[hookIndex] = hookStates[hookIndex]?hookStates[hookIndex]:(typeof initialState === 'function')?initialState():initialState
...
export function useReducer(reducer,initialState,init){
  hookStates[hookIndex] = hookStates[hookIndex]?hookStates[hookIndex]:(typeof init === 'function' && init)?init(initialState):initialState
  let currentIndex = hookIndex
  function dispatch(action){
    let oldState = hookStates[currentIndex]
    let newState = typeof reducer === 'function'?reducer(oldState,action):action
    hookStates[currentIndex] = newState
    scheduleUpdate()
  }
  hookIndex++
...
export function useMemo(factory,deps){
  if (hookStates[hookIndex]){
    ...
    let same = oldDeps.every((item,index) => item === deps[index])
    if (same){
      hookIndex++
      return oldMemo
    }else {
...
export function useEffect(callback,deps){
  ...
  if (hookStates[currentIndex]){
    let [destroy, oldDeps] = hookStates[currentIndex]
    let same = deps.every((item,index) => item === oldDeps[index])
    if (!same){
      destroy && destroy()
      let timer = setTimeout(() => {
        let newDestroy = callback()
        hookStates[currentIndex] = [newDestroy,deps]
        clearTimeout(timer)
      }, 0);
    }
    hookIndex++
...
function createElement(type,props,vdom){
  ...
  }else if(type.type === REACT_FRAGMENT){
...
function updateElement(oldVdom,newVdom){
  ...
  if(type === REACT_TEXT){
    ...
    //渲染CONTEXT组件
    updateFragMentComponent(oldVdom,newVdom)
...
function updateFragMentComponent(oldVdom,newVdom){
  let currentDOM = findDOM(oldVdom)
  if (!currentDOM) return
  newVdom.dom = createDOM
  updateChildren(currentDOM,oldVdom.props.children,newVdom.props.children)
}
...
function updateMemoComponent(oldVdom,newVdom){
  ...
  let same = compare(props,prevProps)
  //这个里面缺少一个逻辑就是hook改变了不管属性有么有变化都会强制更新
  if (!same){
    let newRenderDom = functionComponent(props)
